...
import { SiTailwindcss, SiNextdotjs, SiTypescript, SiHtml5, SiCss3, SiJavascript, SiWasmer } from 'react-icons/si';
...
const IconDisplay = () => {
  return (
    <div className="flex flex-wrap gap-6 sm:gap-8 md:gap-10 lg:gap-12 justify-center items-center">
      {icons.map((icon, index) => (
        <div key={index} className="flex flex-col items-center mt-4">
          {icon.component}
          <span className="mt-2 text-white text-sm sm:text-base md:text-lg lg:text-xl">{icon.name}</span>
        </div>
      ))}
    </div>
  );
